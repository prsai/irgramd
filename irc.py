...
class IRCHandler(object):
    ...
    async def send_privmsg(self, user, source_mask, target, msg, timestamp=None):
        ...
        tags, msg = self.set_history_timestamp(msg, timestamp, user)
    ...
    def render_timestamp(self, timestamp):
        hist_fmt = self.conf['hist_timestamp_format']
        try:
            tsp = format_timestamp(hist_fmt, self.conf['timezone'], timestamp)
            sep = ' ' if tsp else ''
        except:
            if not self.hist_fmt_warning:
                self.logger.warning('Option "hist_timestamp_format" or "timezone" not valid, timestamps not set')
                self.hist_fmt_warning = True
            tsp = ''
            sep = ''
        return tsp, sep

    def set_history_timestamp(self, text, timestamp, user):        
        tags = ''
        res = text
        if timestamp:
            action = (text[:7] == '\x01ACTION')
            ...
            if 'server-time' in user.extensions:
                ...
                tsp, sep = self.render_timestamp(timestamp)
                res = text.format(sep, tsp)
            else:
                res = '{}{}{}'.format(*self.render_timestamp(timestamp), text)
        return tags, res
